...
object ConsignmentItemViewModel {
  ...
  def apply(consignmentItem: ConsignmentItemType03): ConsignmentItemViewModel =
    new ConsignmentItemViewModel(
      ...
      packagesType = consignmentItem.Packaging.map(_.asString).seperateEntities,
      descriptionOfGoods = consignmentItem.Commodity.descriptionOfGoods,
      previousDocuments = consignmentItem.PreviousDocument.map(_.asString).seperateEntities,
      supportingDocuments = consignmentItem.SupportingDocument.map(_.asString).seperateEntities,
      ...
      additionalReferences = consignmentItem.AdditionalReference.map(_.asString).seperateEntities,
      additionalInformation = consignmentItem.AdditionalInformation.map(_.asString).seperateEntities,
      additionalSupplyChainActors = consignmentItem.AdditionalSupplyChainActor.map(_.asString).seperateEntities,
      supplyChainActorId = consignmentItem.AdditionalSupplyChainActor.map(_.identificationNumber).seperateEntities,
      transportDocuments = consignmentItem.TransportDocument.map(_.asString).seperateEntities,
      ...
      dangerousGoods = consignmentItem.Commodity.DangerousGoods.map(_.asString).seperateEntities,
