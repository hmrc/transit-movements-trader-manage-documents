...
object ConsignmentItemViewModel {
  ...
  def apply(consignmentItem: ConsignmentItemType03): ConsignmentItemViewModel =
    new ConsignmentItemViewModel(
      ...
      packagesType = consignmentItem.Packaging.map(_.asString).separateEntities,
      descriptionOfGoods = consignmentItem.Commodity.descriptionOfGoods,
      previousDocuments = consignmentItem.PreviousDocument.map(_.asString).separateEntities,
      supportingDocuments = consignmentItem.SupportingDocument.map(_.asString).separateEntities,
      ...
      additionalReferences = consignmentItem.AdditionalReference.map(_.asString).separateEntities,
      additionalInformation = consignmentItem.AdditionalInformation.map(_.asString).separateEntities,
      additionalSupplyChainActors = consignmentItem.AdditionalSupplyChainActor.map(_.asString).separateEntities,
      supplyChainActorId = consignmentItem.AdditionalSupplyChainActor.map(_.identificationNumber).separateEntities,
      transportDocuments = consignmentItem.TransportDocument.map(_.asString).separateEntities,
      ...
      dangerousGoods = consignmentItem.Commodity.DangerousGoods.map(_.asString).separateEntities,
