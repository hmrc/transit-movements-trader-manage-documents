...
object ConsignmentItemViewModel {
  ...
  def apply(ie015: CC015CType, houseConsignment: CUSTOM_HouseConsignmentType03, consignmentItem: CUSTOM_ConsignmentItemType03): ConsignmentItemViewModel =
    new ConsignmentItemViewModel(
      ...
      previousDocuments = consignmentItem.PreviousDocument.map(_.asSlashSeparatedString).takeSample,
      supportingDocuments = consignmentItem.SupportingDocument.map(_.asSlashSeparatedString).takeSample,
      ...
      additionalReferences = consignmentItem.AdditionalReference.map(_.asSlashSeparatedString).takeSample,
      additionalInformation = consignmentItem.AdditionalInformation.map(_.asSlashSeparatedString).takeSample,
      additionalSupplyChainActors = consignmentItem.AdditionalSupplyChainActor.map(_.asString).takeSample,
      supplyChainActorId = consignmentItem.AdditionalSupplyChainActor.map(_.identificationNumber).take3(_.semiColonSeparate).appendPeriod,
      transportDocuments = consignmentItem.TransportDocument.map(_.asSlashSeparatedString).takeSample,
      ...
      departureTransportMeans = houseConsignment.DepartureTransportMeans.map(_.asString).takeSample,
