...
object ConsignmentItemViewModel {
  ...
  def apply(houseConsignment: CUSTOM_HouseConsignmentType03, consignmentItem: CUSTOM_ConsignmentItemType03): ConsignmentItemViewModel =
    new ConsignmentItemViewModel(
      ...
      additionalReferences = consignmentItem.AdditionalReference.map(_.asSlashSeparatedString).semiColonSeparate.appendPeriod,
      additionalInformation = consignmentItem.AdditionalInformation.map(_.asSlashSeparatedString).semiColonSeparate.appendPeriod,
      additionalSupplyChainActors = consignmentItem.AdditionalSupplyChainActor.map(_.asString).take3(_.semiColonSeparate).appendPeriod,
      ...
      transportDocuments = consignmentItem.TransportDocument.map(_.asSlashSeparatedString).semiColonSeparate.appendPeriod,
      ...
      departureTransportMeans = houseConsignment.DepartureTransportMeans.map(_.asSlashSeparatedString).semiColonSeparate.appendPeriod,
