...
class ConsignmentItemViewModelSpec extends SpecBase with DummyData {
  ...
  "must map data to view model" - {
    ...
    val result = ConsignmentItemViewModel(ie015, houseConsignmentType03, consignmentItemType03)
    ...
    "descriptionOfGoods" - {
      ...
      "must be description of goods for the rest of the other consignment items in HC" in {
        ConsignmentItemViewModel(ie015, houseConsignmentType03, consignmentItemType03.copy(goodsItemNumber = 2)).descriptionOfGoods mustBe "dog"
      }
    }
    ...
    "supplementaryUnits returns" - {
      ...
      "ie015 data for the matching consignment item" in {
        val ie015 = IE015(
          limitDate = None,
          supplementaryUnits = Map[BigInt, Option[BigDecimal]](
            BigInt(1) -> Some(BigDecimal(4.8)),
            BigInt(2) -> Some(BigDecimal(7.6)),
            BigInt(3) -> Some(BigDecimal(9.5))
          )
        )
        ...
        val viewModel = ConsignmentItemViewModel(
          ...
          consignmentItemType03.copy(declarationGoodsItemNumber = 1)
        )
      ...
      "empty when there is no matching consignment item by declarationGoodsItemNumber" in {
        ...
        val viewModel = ConsignmentItemViewModel(
          ...
          consignmentItemType03.copy(declarationGoodsItemNumber = 4)
        )
      ...
      "empty when there is no supplementaryUnits" in {
        val ie015 = IE015(
          limitDate = None,
          supplementaryUnits = Map.empty[BigInt, Option[BigDecimal]]
        )
