...
class FormsSpec extends SpecBase with DummyData {
  ...
  "security.table_1.forms" - {
    ...
    "when consignment items are present" - {
      val pageNumberDetails: PageNumberDetails =
        PageNumberDetails(securityDefined = true, consignmentDefined = true)
      "render the template with page number using a ref-id from the consignment page " in {
        val html: XmlFormat.Appendable = table_1.forms(
          pageNumberDetails: PageNumberDetails
        )
        contentAsString(html) must include("""<fo:page-number-citation ref-id="A-last-page"/>""")

      }
    }
    "when consignment items are not present" - {
      val pageNumberDetails: PageNumberDetails =
        PageNumberDetails(securityDefined = true, consignmentDefined = false)
      "render the template without a last-page reference" in {
        val html: XmlFormat.Appendable = table_1.forms(
          pageNumberDetails: PageNumberDetails
        )
        contentAsString(html) mustNot include("""<fo:page-number-citation ref-id="A-last-page"/>""")
      }
    }
  }
