...
class ConsignorConverterSpec extends AnyFreeSpec with Matchers with ValidatedMatchers with ValidatedValues {
  ...
  "toViewModel" - {
    ...
    "must return a view model when the country code is found in the reference data" in {
      ...
      result.valid.value mustEqual viewmodels.Consignor(
        "name",
